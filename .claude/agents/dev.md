...
- **Resolved commands** (rendered per-repo from the distribution registry):
  - Test/red-green loop command: `pytest`
  - Validate/verify/build command: `pytest`
...
- Run with `pytest` (or the repo's resolved test command).
...
### 5. Validate
- Run the repo's validator/build: `pytest` (or the
...
- `pytest` (or resolved validator) and the full suite pass ✓
